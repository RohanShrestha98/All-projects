  ...
  <Alert.Root onOpenChange={open => open === false && closeHandler()}>
    ...
    <Alert.Portal>
      ...
      <Alert.Content className="modal-content xs:px-4 xs:gap-4 flex max-w-md flex-col gap-5 px-7 py-5">
        ...
        <div className="xs:gap-4 flex gap-4 self-end md:gap-6">
          <Alert.Cancel asChild>
            <button className="bg-danger text-white px-6 w-44  h-9 rounded border border-danger hover:bg-white hover:text-danger">
              {t('cancel')}
            </button>
          </Alert.Cancel>
          <Alert.Action asChild onClick={clickHandler}>
            <button className="bg-primary text-white px-6 w-48  h-9 rounded border border-primary hover:bg-white hover:text-primary">
              {t('yesDeleteData')}
            </button>
          </Alert.Action>
        </div>
