...
import { productmasterbycustomer } from "./Tableheader";
import "./Table.scss";
import { BiEdit, BiTrash } from "react-icons/bi";
import { TFunction } from "i18next";
import { withTranslation } from "react-i18next";

interface ProductMasterbyCustomerTableProps {
  t: TFunction;
}
...
function ProductMasterbyCustomerTable({
  t,
}: ProductMasterbyCustomerTableProps) {
    ...
    <div className="boxtable">
      <Box sx={{ height: 635, width: "100%" }} className="">
        <DataGrid
          className="table"
          rows={rows}
          columns={productmasterbycustomer.concat(actionColumn)}
          initialState={{
            pagination: {
              paginationModel: {
                pageSize: 9,
...
export default withTranslation()(ProductMasterbyCustomerTable);
