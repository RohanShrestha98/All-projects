...
export default function CourseGroup() {
    ...
    const { data, isLoading, isError } = useCourseGroupData(searchText, selectedField, pageSize, page)
                ...
                cell: (info) => {
                    return (
                        <div className="flex gap-2 text-base justify-center">
                            ...
                            <DeleteModal asChild desc={"Are you sure you want to delete this category"} title={"Delete Category"} id={info?.row?.original?.id}>
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <FilterSearch searchText={searchText} setSelectedField={setSelectedField} inputPlaceholder={"Search Course Group"} setSearchText={setSearchText} select={false} selectPlaceholder={"Select Course"} />
                <AddCourseGroupModal asChild>
                    <div>
                        <TopButton buttonName={"Add Course Group"} className={""} handleButtonClick={() => { }} />
                    </div>
                </AddCourseGroupModal>
            </div>
            <div>
