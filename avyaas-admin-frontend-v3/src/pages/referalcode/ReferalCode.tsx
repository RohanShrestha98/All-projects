...
export default function ReferalCode() {
    ...
    const { data, isLoading, isError } = useReferalCodeData(searchText, selectedField, pageSize, page)
                ...
                cell: (info) => {
                    return (
                        <div className="flex gap-2 text-base justify-center">
                            ...
                            <DeleteModal asChild desc={"Are you sure you want to delete this referal code"} title={"Delete Referal Code"} id={info?.row?.original?.id}>
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <FilterSearch searchText={searchText} setSelectedField={setSelectedField} options={courseOptions} inputPlaceholder={"Search referal code"} setSearchText={setSearchText} selectPlaceholder={"Select Course"} />
                <AddReferalCodeModal asChild>
                    <div>
                        <TopButton buttonName={"Add Code"} className={""} handleButtonClick={() => { }} />
                    </div>
                </AddReferalCodeModal>
            </div>
            <div>
