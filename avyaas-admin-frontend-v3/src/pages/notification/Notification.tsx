...
export default function Notification() {
    ...
    const { data, isLoading, isError } = useNotificationData(searchText, selectedField, pageSize, page)
    ...
    const handlePublish = async (id) => {
        try {
            const response = await publishNotificationMutation.mutateAsync(["post", `${id}`])
        ...
        () => [
            ...
            },
            {
                ...
                cell: info => {
                    return (
                        <p className="flex items-center gap-1">
                            {moment(info?.row?.original?.scheduledDate).format("MMM Do YYYY, h:mm:ss a")}
                        </p>
                    )
                },
                ...
                cell: (info) => {
                    return (
                        <div className="flex gap-2 text-base justify-center">
                            <FiSend onClick={() => handlePublish(info?.row?.original?.id)} className="text-green-600 cursor-pointer" />
                            <AddNotificationModal asChild edit editData={info?.row?.original}>
                                <FiEdit2 className="text-[#4365A7] cursor-pointer" />
                            </AddNotificationModal>
                            <DeleteModal asChild desc={"Are you sure you want to delete this notification"} title={"Delete Notification"} id={info?.row?.original?.id}>
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <FilterSearch searchText={searchText} setSelectedField={setSelectedField} options={courseOptions} inputPlaceholder={"Search Notification"} setSearchText={setSearchText} selectPlaceholder={"Select Course"} />
                <AddNotificationModal asChild>
                    <div>
                        <TopButton buttonName={"Add Notification"} className={""} handleButtonClick={() => { }} />
                    </div>
                </AddNotificationModal>
            </div>
            <div>
