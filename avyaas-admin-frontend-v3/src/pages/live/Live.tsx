...
export default function Live() {
    ...
    const { data, isLoading, isError } = useLiveData(searchText, selectedField, pageSize, page)
        ...
        () => [
                ...
                cell: info => {
                    return (
                        <p className="flex items-center gap-1">
                            {moment(info?.row?.original?.start_time).format("MMM Do YYYY, h:mm:ss a")}
                        </p>
                    )
                },
            ...
            },
            {
                ...
                cell: info => {
                    return (
                        <p>
                            {info?.row?.original?.duration + " days"}
                        </p>
                    )
                },
                ...
                cell: (info) => {
                    return (
                        <div className="flex gap-2 text-base justify-center">
                            ...
                            <DeleteModal asChild desc={"Are you sure you want to delete this live"} title={"Delete Live"} id={info?.row?.original?.id}>
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <FilterSearch searchText={searchText} setSelectedField={setSelectedField} options={liveGroupOptions} inputPlaceholder={"Search Live "} setSearchText={setSearchText} selectPlaceholder={"Select Live Group"} />
                <AddLiveModal asChild>
                    <div>
                        <TopButton buttonName={"Add Live"} className={""} handleButtonClick={() => { }} />
                    </div>
                </AddLiveModal>
            </div>
            <div>
