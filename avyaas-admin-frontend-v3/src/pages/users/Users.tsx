...
export default function Users() {
  ...
  const { data, isLoading, isError } = useStudentData(searchText, selectedField, pageSize, page)
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: info => {
          return (
            <div className="flex items-center gap-1">                            <p className="flex items-center gap-1">
              {info?.row?.original?.firstName === "" ? "-" : info?.row?.original?.firstName + " "}
      ...
      },
      {
        ...
        cell: info => {
          return (
            <p>{info?.row?.original?.email === "" ? "-" : info?.row?.original?.email}</p>
          )
        },
      ...
      },
      {
        ...
        cell: info => {
          return (
            <p>{info?.row?.original?.phone === "" ? "-" : info?.row?.original?.phone}</p>
          )
        },
      ...
      },
      {
        ...
        cell: info => {
          return (
            <p className={`inline-block text-xs px-4 cursor-default rounded-full py-[2px] font-medium ${info?.row?.original?.verified ? "text-white bg-[#027A48]" : "text-white bg-red-500"}`}>
        ...
        cell: (info) => {
          return (
            <div className="flex gap-2  justify-center">
              <AddManualPaymentModal asChild edit editData={info?.row?.original}>
    ...
    <div className="p-4 flex flex-col gap-4">
      ...
      <div>
        ...
        <ReactTable
          ...
          currentPage={1}
          totalPage={1}
          emptyMessage="Oops! No Users available right now."
        />
      </div>
    </div>
  );
