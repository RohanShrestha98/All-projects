...
export default function Units() {
    ...
    const { data, isLoading, isError } = useUnitData(searchText, selectedField, pageSize, page)
                ...
                cell: info => {
                    return (
                        <div className="flex items-center gap-1">
                            {
                                info?.row?.original?.thumbnail ? <img className="h-8 w-8 object-cover rounded-full" src={info?.row?.original?.thumbnail} alt="" />
                ...
                cell: (info) => {
                    return (
                        <div className="flex gap-2 text-base justify-center">
                            ...
                            <DeleteModal asChild desc={"Are you sure you want to delete this unit"} title={"Delete Unit"} id={info?.row?.original?.id}>
    ...
    const handleUpdateUnitPosition = async () => {
        ...
        try {
            const response = await positionUpdateMutation.mutateAsync(["patch", "", postData])
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <FilterSearch searchText={searchText} setSelectedField={setSelectedField} options={subjectOptions} inputPlaceholder={"Search Unit"} setSearchText={setSearchText} selectPlaceholder={"Select Subject"} />
                <div className="flex items-center gap-2">
                    {
                        selectedField && <Button buttonName={"Update Unit Position"} handleButtonClick={handleUpdateUnitPosition} />
                    }
                    <AddUnitModal asChild>
                        <div>
                            <TopButton buttonName={"Add Unit"} className={""} handleButtonClick={() => { }} />
                        </div>
                    </AddUnitModal>
                </div>
            </div>
