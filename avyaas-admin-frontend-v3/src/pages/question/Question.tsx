...
import { useCourseData, useQuestionBankData, useQuestionData, useSubjectData } from "@/hooks/useQueryData";
...
export default function Question() {
    ...
    const { data, isLoading, isError } = useQuestionData(searchText, selectedSubject, pageSize, page)
    ...
    const onSubmitHandler = async () => {
        ...
        try {
            const result = await assignQuestionMutation.mutateAsync(["post", "", postData]);
    ...
    const columns = useMemo(
        () => [
            ...
            },
            {
                ...
                cell: info => {
                    return (
                        <p>{info?.row?.original?.course?.tile ?? "-"}</p>
                    )
                },
                ...
                cell: (info) => {
                    return (
                        <div className="flex gap-2 text-base justify-center">
                            ...
                            <DeleteModal asChild desc={"Are you sure you want to delete this question"} title={"Delete Question"} id={info?.row?.original?.id}>
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <FilterSearch searchText={searchText} setSelectedField={setSelectedField} setSelectedSubject={setSelectedSubject} subjectOptions={subjectOptions} options={courseOptions} inputPlaceholder={"Search question"} setSearchText={setSearchText} selectPlaceholder={"Select Course"} />
                <div className="flex gap-2">
                    {
                        selectedRows?.length ? < >
                            <CustomSelect className={"w-[220px]"} options={questionBankOptions} label={""} placeholder={"Select a question set"} setSelectedField={setSelectedQuestionBank} />
                        </> : <></>
                    }
                    {selectedQuestionBank && selectedRows?.length ?
                        <Button buttonName={"Assign to Question Set"} className={"w-full "} handleButtonClick={() => onSubmitHandler()} icon={""} />
                        : <AddQuestionModal asChild>
                            <div>
                                <TopButton buttonName={"Add Question"} className={""} handleButtonClick={() => { }} />
                            </div>
                        </AddQuestionModal>
                    }
