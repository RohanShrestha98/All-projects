...
export default function QuestionBank() {
    ...
    const { data, isLoading, isError } = useQuestionBankData(searchText, selectedField, pageSize, page)
                ...
                cell: (info) => {
                    return (
                        <div className="flex gap-2 text-base justify-center">
                            <LuEye onClick={() => navigate("/question-bank/preview-questions", {
                                state: {
                                    data: info?.row?.original
                                }
                            })} className="text-orange-600 text-md cursor-pointer" />
                            <RiContactsBookUploadLine onClick={() => navigate("/question-bank/question", {
                                state: {
                                    data: info?.row?.original
                                }
                            ...
                            <DeleteModal asChild desc={"Are you sure you want to delete this question set"} title={"Delete Question Set"} id={info?.row?.original?.id}>
    ...
    return (
        <div className="p-4 flex flex-col gap-4">
            <div className="flex justify-between items-center">
                <FilterSearch searchText={searchText} setSelectedField={setSelectedField} options={courseOptions} inputPlaceholder={"Search question set"} setSearchText={setSearchText} selectPlaceholder={"Select Course"} />
                <AddQuestionBankModal asChild>
                    <div>
                        <TopButton buttonName={"Add Question Set"} className={""} handleButtonClick={() => { }} />
                    </div>
                </AddQuestionBankModal>
            </div>
            <div>
