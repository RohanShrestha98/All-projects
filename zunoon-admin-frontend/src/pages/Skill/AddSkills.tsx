import React, { useState } from "react";
import { useNavigate } from "react-router-dom";

import SkillForm from "./components/SubjectForm/SkillForm";

import Button from "../../components/Button/Button";
import http from "../../utils/http";
import toast from "../../utils/toast";

import "./AddSkills.scss";

import config from "../../config";
import { withTranslation } from "react-i18next";
const skillApi = config.endpoints.api.skill;

function AddSkills({ t }) {
  const navigate = useNavigate();
  const [isSubmitting, setIsSubmitting] = useState<boolean>(false);

  const handleClickSubmit = data => {
    setIsSubmitting(true);
    http
      .POST_FILE(skillApi.create, data)
      .then(res => {
        toast.success(`  ${t("th_skill")} ${t("added_successfully")}!`);
        setTimeout(() => {
          navigate("../");
        }, 1000);
        setIsSubmitting(false);
      })
      .catch(error => {
        let resErrors = error?.response?.data?.errors;
        toast.error(Object.values(resErrors)?.toString() || "Error occured");
        setIsSubmitting(false);
      })
      .finally(() => {
        setIsSubmitting(false);
      });
  };

  return (
    <>
      <div className="page_header">
        <h4 className="page_title">{`${t("sidebar_skills")} | ${t("add")} ${t(
          "sidebar_skills",
        )}`}</h4>

        <div className="button_wrapper ">
          <Button
            type="button"
            buttonName={`< ${t("back")}`}
            color="success"
            clickHandler={() => navigate("../")}
          />
        </div>
      </div>

      <div className="add_subject_form">
        <SkillForm
          editform={false}
          handleClickSubmit={data => handleClickSubmit(data)}
          isSubmitting={isSubmitting}
        />
      </div>
    </>
  );
}

export default withTranslation()(AddSkills);
