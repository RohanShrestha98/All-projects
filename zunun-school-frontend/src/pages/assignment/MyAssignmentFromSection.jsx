...
export default function MyAssignmentFromSection({ active, location }) {
  const [searchText, setSearchText] = useState("");
  const { data, isError, isLoading } = useQueryData(
    ["section-assignment-list", location, searchText],
    `api/v1/assignment/list/?section=${
      location && location
    }&&query=${searchText}`,
  );
  const useEventDeleteMutation = () =>
    useMutate(["section-assignment-list"], "api/v1/assignment/delete/");
