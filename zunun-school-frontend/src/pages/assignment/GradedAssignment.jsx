...
import noGraded from "../../assets/images/noGraded.png"

export default function GradedAssignment({ active }) {
  const [searchText, setSearchText] = useState("");
  const { data, isLoading, isError } = useQueryData(
    ["graded-list", searchText],
    `api/v1/assignment/student-list/?status=graded&&query=${searchText}`,
  );
    ...
    <div>
      <div className="w-full  bg-white mt-4 rounded-t-xl  px-8 py-4 flex justify-between items-center border-b-[1px] border-gray-6">
        <p className="font-semibold">
          Total Assignment :
          {active === "graded" && <span> {data?.data?.length ?? 0}</span>}
        </p>
      ...
      {active === "graded" &&
        (assignmentPermission.includes("/assignment/list/") ||
          ...
          <>
            <ReactTable
              emptyImage={noGraded}
              title={"No assignments graded yet"}
              loading={isLoading}
              error={isError}
              columns={columns}
              data={data?.data ?? []}
